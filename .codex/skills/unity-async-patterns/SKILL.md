...
description: Reference awaitable cancellation, thread context, coroutine lifetime and asset handles when that topic is requested in Absolute Zero; adapt examples to installed APIs and current project ownership.
---

# unity-async-patterns

Read [project compatibility](../../UNITY_COMPATIBILITY.md) before using the examples. Read AI_TARGET_ARCHITECTURE for async changes. AsTask examples need the custom bridge; destroy cancellation alone does not cover network despawn or round reset.
...
- [async cancellation patterns](references/async-cancellation-patterns.md)
