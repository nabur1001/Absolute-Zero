...
GOTCHA: `SceneManager.SetActiveScene` determines which scene's lighting settings apply and where newly instantiated objects are placed. Forgetting this causes objects spawning in the persistent scene (wrong lightmaps, wrong navmesh). This scaffold assumes a bootstrap scene in Build Settings. Addressable scene loading has separate catalog requirements; follow current NGO scene ownership in this project.
