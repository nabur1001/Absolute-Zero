...
public class SeededRandom
{
    ...
    public static int DeriveSeed(int masterSeed, string subsystem)
        {
        // Explicit FNV-1a over UTF-16 code units, seeded by masterSeed.
        // Keep this algorithm/input contract stable for persisted seeds.
        unchecked
        {
            uint hash = 2166136261u ^ (uint)masterSeed;
            foreach (char c in subsystem) { hash ^= c; hash *= 16777619u; }
            return (int)hash;
        }
    }
}
