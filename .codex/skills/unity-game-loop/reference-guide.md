> Imported reference material. Read [project compatibility](../../UNITY_COMPATIBILITY.md) before adapting examples. Examples are not installed game systems or fully compiled samples.
...
GOTCHA: Transitions that actually wait for loading or presentation may be asynchronous. Preserve the existing phase owner and choose synchronous, coroutine, or Awaitable flow according to the task and lifecycle.
...
GOTCHA: `AnimationCurve.Evaluate(time)` evaluates authored key times; this example chooses normalized time (0-1). You must clearly document what 0 and 1 represent: level start/end, player death count range, or session elapsed time ratio. Ambiguity here causes difficulty curves that feel wrong and are impossible to debug.
