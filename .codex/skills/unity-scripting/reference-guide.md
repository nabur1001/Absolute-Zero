...
> Compatibility: Awaitable.AsTask in these examples requires the custom bridge in unity-scripting/references/coroutines-and-async.md; it is not a Unity built-in. Consume the Awaitable once. See the compatibility contract.
...
|-------------|---------|-----|
| `GetComponent<T>()` in `Update()` | Repeats component lookup; allocation depends on API and context | Cache in `Awake()` |
| `GameObject.Find()` in `Update()` | Expensive string search every frame | Cache reference or use serialized field |
| Repeated `new Vector3()` in hot paths | Value-type construction is not inherently a managed allocation | Construct the required value; optimize only measured hot paths |
