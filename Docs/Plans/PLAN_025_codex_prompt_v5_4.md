# PLAN_025 v5.4 Verification Prompt — Codex Review #9
...
9. **`PlayerSpawnManager.cs:168-175`**: Current `OnClientDisconnectCallback` immediately calls `DespawnPlayerForClient` → Despawn+Destroy. **Must be disabled in Multi/Solo to prevent race with MatchRoster**
...
13. **`TemperatureSystem.cs:11`**: `public const float DEFAULT_FAN_SPEED = 1f;` — round reset must use this value, NOT 0
14. **`RoundLifecycleService.cs:31,43-44`**: `player.FanSpeed.Value = TemperatureSystem.DEFAULT_FAN_SPEED;` — current reset uses 1f
15. **`PlayerInventory.cs:52`**: `if (slot.IsUnlimited) return;` — Multi windbreaker must be `IsUnlimited=false`
16. **Unity Lobby permissions**: Each player can only modify own PlayerData. Host can READ Private data but NOT write others'.
17. **`BuffItemDataSO.cs:9-11`**: `float ImmediateTempDelta; float DelayedTempDelta; int DelayTurns = 1;` — Buff is self-target, signed (>0=heal, <0=damage)
18. **`DebuffItemDataSO.cs:9-12`**: `float ImmediateTempDelta; float DelayedTempDelta; int DelayTurns = 1; DamageFilter AttackFilter;` — Debuff is opponent-target, signed
19. **`PlayerState.cs:24,33,42`**: `IsReady`, `HasSelectedItem`, `CosmeticDataNV` — NVs NOT covered by ISeatStateAccessor (turn-progress/cosmetic, not gameplay state)
20. **`NetworkSessionCoordinator.cs:293`**: `_networkRuntime.StartHost(relayResult.Value.ServerData)` — StartHost called before Relay code distribution
...
## Review #8 Fixes Applied in v5.4

| # | Issue | Fix |
|---|-------|-----|
| B1 | PlayerSpawnManager.OnClientDisconnectCallback races MatchRoster — Despawn before NV capture | **Disconnect 단일 소유자**: Multi/Solo에서 PlayerSpawnManager callback 비활성화. MatchRoster가 유일한 handler. Capture→Roster→Despawn 순서 보장 |
| B2 | Applicator Shell writes PlayerState.Temperature.Value directly — fails for disconnected targets | **Applicator Shell ISeatStateAccessor 경유**: 모든 NV write를 ISeatStateAccessor로 통일. PlayerState 직접 참조 금지 |
| B3 | ItemEffectRuleSnapshot missing signed ImmediateTempDelta for Buff/Debuff (Soda -5/+15, Samgyetang +3/-7) | `ImmediateTempDelta`(signed) + `IsSelfTarget` 추가. **21종 SO→Snapshot From() 매핑 표** + Buff/Debuff Resolver 규칙 |
| B4 | Round reset SetFanSpeed(seat,0) but actual default is TemperatureSystem.DEFAULT_FAN_SPEED=1f | `SetFanSpeed(seat, DEFAULT_FAN_SPEED)` (=1f) + `SetIsFanActive(seat, false)` |
| P1 | Gate 1 lacks StartHost ordering — token table/approval callback must precede StartHost | **StartHost 전 사전조건**: ① token 등록 완료 ② approval callback 등록 ③ StartHost() ④ Gate 1 통과 후 Relay 코드 배포 |
| P2 | ISeatStateAccessor "전체 NV 커버" but IsReady/HasSelectedItem/CosmeticDataNV excluded | ISeatStateAccessor 범위 = **"authoritative gameplay state during disconnect grace"**. 제외 NV 4종 offline 기본값 명시 |
...
4. **Disconnect/reconnect correctness**: DontDestroyWithOwner=true timing, **single disconnect handler** (PlayerSpawnManager disabled), ISeatStateAccessor for ALL operations, hydrate commit order
5. **Formula accuracy**: Equalize (target.temp=user.temp, NOT average), Recovery (HealPerUse[] lookup), Tarot (IsReady state), **Buff/Debuff (signed ImmediateTempDelta + DelayedTempDelta)**
6. **DTO serialization**: Writer+reader validation, manual element serialization, MTU bounds
7. **Token security**: RandomNumberGenerator, 2-stage gate (token table + approval callback → StartHost → Lobby gate → Relay code → NGO gate → scene), Lobby permission model
...
10. **Completeness**: All 192 checkboxes are implementable, no missing steps
11. **ISeatStateAccessor scope**: Covers authoritative gameplay state (NOT all NVs). IsReady/HasSelectedItem/CosmeticDataNV/SyncedPlayerIndex excluded with offline defaults
12. **ScheduledEffect non-duplication**: Only in BuffDebuffSystem central dict, not in SeatRuntimeState
13. **Applicator Shell accessor usage**: All NV writes go through ISeatStateAccessor, no PlayerState direct reference
14. **Round reset values**: FanSpeed=DEFAULT_FAN_SPEED(1f), IsFanActive=false, Temperature=37, LifeState=Alive
15. **21-item From() mapping**: All item SO types mapped to ItemEffectRuleSnapshot with correct field assignments
...
## FULL PLAN TEXT (192 checkboxes — mechanically copied from plan file)

# PLAN_025 — Multi Mode Full Implementation (v5.4 — Codex Review #8 반영)

> **Status:** 📋 Planning (v5.4 — Review #1~#8 전체 반영, 재검증 대기)
...
- [ ] A-3e. **Disconnect 처리 — 단일 소유자 (B1 v5.4)**:
  ...
  **해결: Disconnect 처리 단일 소유자**
  - Multi/Solo 모드 진입 시 PlayerSpawnManager.OnClientDisconnectCallback **구독 해제**
    (또는 MatchRoster가 등록된 상태에서 PlayerSpawnManager callback이 no-op 되도록 guard)
  - MatchRoster가 유일한 OnClientDisconnectCallback handler
  - 1v1 모드는 기존 PlayerSpawnManager 동작 유지 (회귀 방지)
...
- [ ] B-3b. `ItemEffectRuleSnapshot` readonly struct (B3 해결 — **계산 규칙 캡처, 결과 아님**):
  ...
  **21종 SO → ItemEffectRuleSnapshot From() 매핑 (B3 v5.4)**:
  ```
  | SO Type | EffectKind | ImmediateTempDelta | IsSelfTarget | BaseDamage | EqualizeToUserTemp | HealPerUse | HasScheduled | ScheduledValue | ScheduledDelay | 비고 |
  |---------|------------|-------------------|-------------|------------|-------------------|-----------|-------------|---------------|---------------|------|
  | AttackItemDataSO (일반) | DirectDamage | 0 | false | so.BaseDamage | false | - | false | - | - | WaterGun(8°), BuldakNoodles(20°) |
  | AttackItemDataSO (Equalize) | Equalize | 0 | false | 0 | so.EqualizeToUserTemp=true | - | false | - | - | target.temp=user.temp |
  | RecoveryItemDataSO | Recovery | 0 | true | 0 | false | so.HealPerUse.ToArray() | false | - | - | HealPerUse[Clamp(Max-Rem,0,len-1)] |
  | DefenseItemDataSO | Defense | 0 | true | 0 | false | - | false | - | - | IsDefense=true |
  | BuffItemDataSO | Buff | so.ImmediateTempDelta | true | 0 | false | - | !=0 | so.DelayedTempDelta | so.DelayTurns | Soda: -5/+15 |
  | DebuffItemDataSO | Debuff | so.ImmediateTempDelta | false | 0 | false | - | !=0 | so.DelayedTempDelta | so.DelayTurns | Samgyetang: +3/-7 |
  | SpecialItemDataSO (Fan) | FanControl | 0 | varies | 0 | false | - | so.DelayTurns>0 | so.EffectValue | so.DelayTurns | CoolBreeze, Upgrade 등 |
  | SpecialItemDataSO (Tarot) | Special | 0 | false | 0 | false | - | false | - | - | RequiresTargetReady=true |
  | SpecialItemDataSO (Other) | Special | 0 | varies | 0 | false | - | false | - | - | Cat(reroll), Claw(steal) |
  ```
  
  **Buff/Debuff Resolver 규칙 (B3 v5.4)**:
  ```
...
- B1 v5.4 (Disconnect single handler): [assessment]
- B2 v5.4 (Applicator ISeatStateAccessor): [assessment]
- B3 v5.4 (Buff/Debuff ImmediateTempDelta + 21-item mapping): [assessment]
- B4 v5.4 (FanSpeed DEFAULT_FAN_SPEED + IsFanActive): [assessment]
- P1 v5.4 (StartHost ordering): [assessment]
- P2 v5.4 (ISeatStateAccessor scope definition): [assessment]
