...
| # | 지적 | v5.4 반영 |
|---|------|---------|
| B1 | PlayerSpawnManager의 OnClientDisconnectCallback이 MatchRoster보다 먼저 실행되면 NV 추출 전 PlayerObject Despawn | **Disconnect 처리 단일 소유자**: PlayerSpawnManager 직접 disconnect callback은 모든 모드에서 제거. DisconnectDispatcher가 destructive lifecycle 단일 소유, 1v1은 default PSM handler 경유. **Capture→Roster→Despawn 순서 보장** 명시 |
...
- [x] A-1a. `NetworkConstants.GameMode` enum: `None=0, OneVsOne=1, Multi=2, Solo=3`
- [x] A-1b. `IGameModeRule` interface (12개 필드)
- [x] A-1c. `GameModeRuleSO : ScriptableObject, IGameModeRule` — 배열 Awake copy
- [x] A-1d. `MatchConfig` 서버 런타임: `{ IGameModeRule Rule, int RequiredPlayerCount, GameMode Mode }`
- [x] A-1e. `MatchConfigNetData : INetworkSerializable` — `{ byte Mode, byte RequiredPlayerCount }`

### A-2. MatchNetworkState — 완전한 NGO 수명주기 (B2 해결)

- [x] A-2a. `MatchNetworkState : NetworkBehaviour` 씬 오브젝트로 배치:
...
- [x] A-2e. OnNetworkDespawn — 이벤트 해제:
  ```csharp
  ...
  ```
- [x] A-2f. OnDestroy — NetworkList Dispose:
  ```csharp
  void OnDestroy() { KillScores?.Dispose(); }
  ```
- [x] A-2g. 서버: scene load 완료 후 `Config.Value = data` 1회 기록 + `KillScores` 초기화 (RequiredPlayerCount 크기, 0 fill)
- [x] A-2h. MCR: `MatchNetworkState` FindAnyObjectByType → 복제 상태 읽어 local MatchConfig 조립

### A-3. Session Scope vs Match Scope Roster

- [x] A-3a. **SessionParticipantTable** (DDOL, Coordinator 소유):
  - Host 시작 시 생성
  - Lobby participant:
    ```csharp
...
- [x] A-3b. **MatchRoster** (서버 전용, MCR 소유):
  ...
  - PlayerIndex = seat 불변
- [x] A-3c. **SeatRuntimeState** 서버 전용 struct (B1 해결):
  ```csharp
...
- [x] A-3d. **Player.prefab 설정 변경 (B1 v5.2)**:
  ```
  ...
  ```
- [x] A-3e. **Disconnect 처리 — 단일 소유자 (B1 v5.4)**:
  ```
  ⚠ 경쟁 조건 방지 (B1 v5.4):
...
- [ ] A-3f. **Reconnect** (B1 해결):
  ...
  ```
- [x] A-3g. **ISeatStateAccessor 인터페이스 (B1 v5.3 + P2 v5.4 — authoritative gameplay state)**:
  ```csharp
  // ⚠ 범위 정의 (P2 v5.4):
...
- [x] A-4a. `PlayerSpawnManager.GetSpawnPosition(byte seatIndex)`: markers[seat].position
- [x] A-4b. 서버 스폰: `MatchRoster.GetSeat(clientId) → seat → GetSpawnPosition(seat)`
  - **Roster hydration 순서:**
...
- [x] A-4c. PlayerSpawnManager scene 판정 수정:
...
- [x] A-4d. `PlayerState.Initialize(byte seat)` — TurnManager ClientId 정렬 경로 제거
- [x] A-4e. PlayerRegistry: `Register(seat, playerState)` seat 기반 lookup

### A-5. 로비 → 다인전 진입

- [x] A-5a. LobbyModeSelectView "다인전" 버튼 → 인원 선택
- [x] A-5b. LobbyPresenter.HandleMultiClicked(int count)
- [x] A-5c. LobbyRoomView 3~4인 슬롯 (기존 dynamic — lobby.MaxPlayers 기반, 코드 변경 불필요)
- [x] A-5d. NetworkSessionCoordinator: CreateLobbyAsync(mode, playerCount), Relay AllocateAsync(playerCount - 1), 씬 분기
- [x] A-5e. 기존 1v1: OneVsOne+2 (기존 시그니처 보존 — SetMatchParameters default + HandleOneVsOneClicked 명시적 설정)
- [x] A-5f. ConnectionApproval 등록

### A-6. GameScene_Multi 씬

- [x] A-6a. GameScene 복사 → index 2
- [x] A-6b. SpawnPoint3D 4개 (Order 0~3)
- [x] A-6c. EnemyPlayer_0/1/2
- [x] A-6d. Visual slot ↔ Match seat 분리:
  ```
  localSeat = myPlayerIndex
  remoteSeatList = allSeats.Where(s != localSeat).OrderBy(s)
  remoteSeatList[i] → EnemyPlayer_{i}
  ```
- [x] A-6e. GameScene_Multi: 복사된 MatchCompositionRoot 확장 + MatchNetworkState(+NetworkObject) + Multi GameModeRuleSO. 씬별 MCR 정확히 1개
- [x] A-6f. GameScene 기존 MatchCompositionRoot 확장 + MatchNetworkState(+NetworkObject) 추가 (1v1용):
...
- [x] A-7a. MatchSnapshot 확장
- [x] A-7b. GameDataBridge 확장
- [x] A-7c. MatchHudPresenter 확장
- [x] A-7d. GameHudRefs 확장
- [x] A-7e. GameHudBuilder 확장
- [x] A-7f. OpponentBarPresenter 확장
- [ ] A-7g. InventoryPresenter 확장 ← Phase B 의존 (B-2c)
- [x] A-7h. RoundResultPresenter 확장
- [x] A-7i. TemperaturePresenter 확장
...
- [x] B0-1a. `LifeState { Alive, Ghost }` NV (PlayerState)
- [x] B0-1b. Roster = 연결 (Connected/Disconnected/TimedOut), LifeState = 게임 (Alive/Ghost)
- [x] B0-1c. **상태 조합 정의** (B3 해결):
  ```
...
- [x] B0-2a. **TryKill(byte victimSeat, DamageSource) — seat 기반 (B1 v5.3)**:
...
- [x] B0-2b. IPlayerTurnCancellation 기본 구현
- [x] B0-2c. 사망 감지: deathQueue → batch 후 FlushDeathQueue → EvaluateRoundMatch
  - Ghost Frost (PrepPhase 즉시): mutation 직후 즉시 FlushDeathQueue + round check
- [x] B0-2d. DamageSource 정규화:
  ```
  ...
  ```
- [x] B0-2e. ScheduledEffect.SourceSeat 추가, Schedule() 시그니처 확장

### B0-3. 킬 스코어 + 승리 + 라운드 리셋

- [x] B0-3a. KillScores (MatchNetworkState 소유)
- [x] B0-3b. SeatSnapshot +killScore, +lifeState
- [x] B0-3c. 공동 승리 SurvivorMask (Shell 계산)
- [x] B0-3d. 라운드 종료: **CountsAsAliveForRoundEnd** ≤ 1 or 전원 Ghost (B3)
- [x] B0-3e. **라운드 리셋 — ISeatStateAccessor 경유 (B1 v5.3)**:
  ```
...
- [x] B-1a. `ActionIntent` readonly struct: { SourceSeat, SlotIndex, ItemId, TargetSeat(255=NoTarget), ReadyServerTick(**int**), _isValid }
  - ⚠ **ReadyServerTick** = `NetworkManager.Singleton.ServerTime.Tick` at PressReadyServerRpc receive (P2 해결)
...
- [x] B-1b. QueuedAction.TargetSeat 추가
- [x] B-1c. PlayerState._pendingIntent 보존 (미니게임 경유 _pendingMiniGameTarget, 재검증)
- [x] B-1d. IPlayerTurnCancellation 확장 (+ClearPendingIntent)
- [x] B-1e. ILocalPlayerCommands + LocalPlayerCommandAdapter 확장 (+TrySelectItemWithTarget)

### B-2. 아이템 타겟

- [x] B-2a. TargetMode enum: Self, SingleTarget
- [x] B-2b. 21종: SingleTarget **13종** (타로 포함), Self 8종 — virtual GetTargetMode() + 4 subclass override
- [ ] B-2c. 드래그-타겟 UI (1v1 자동 타겟) — Phase B-6 scope, deferred
- [x] B-2d. 서버 검증 — TryResolveTargetSeat: Registry 검증, self-target 거부, auto-assign, 미니게임 후 재검증 (Codex 8/10)

### B-3. Value-Type Snapshots (P1 해결)

> **계약**: factory가 모든 배열을 **defensive copy**로 생성. Resolver는 입력 배열을 **변경하지 않음** (readonly 참조 + 문서화된 불변식).

- [x] B-3a. `GameModeRuleSnapshot` readonly struct — IGameModeRule 전체 13속성 value copy, From() factory (Codex 10/10)
- [x] B-3b. `ItemEffectRuleSnapshot` readonly struct (B3 해결 — **계산 규칙 캡처, 결과 아님**) — 7 subclass exhaustive switch, ItemEffectKind enum, defensive copy (Codex 10/10):
  ```csharp
...
- [x] B-3c. `InventorySnapshot` readonly struct (seat별): `{ byte SeatIndex, SlotSnapshot[] Slots }` (Codex 10/10)
  - `SlotSnapshot { short ItemId, bool IsUnlimited, byte RemainingUses }`
  - Factory: defensive `.ToArray()` copy
- [x] B-3d. `ScheduledEffectSnapshot` readonly struct: `{ byte TargetSeat, byte SourceSeat, EffectType Type, float Value, int TurnsRemaining }` (Codex 10/10)

### B-4. Functional Core (B4 해결 — 완전한 delta)

- [x] B-4a. `MatchCombatSnapshot` — **순수 value-type만, defensive copy** + **ISeatStateAccessor read (B2 v5.5)**:
  ```
  ...
  **MatchCombatSnapshotFactory (B2 v5.5 추가)**:
  ...
  ```
- [x] B-4b. `MultiCombatResolution` — **전체 state delta** (B4):
  ```
...
- [x] B-4c. `CombatResolver.ResolveMulti(MatchCombatSnapshot, ActionIntent[])` → `MultiCombatResolution`:
...
- [x] B-4d. **Applicator Shell — ISeatStateAccessor 경유 (B2 v5.4)**:
  ```
...
- [x] B-5a. `CombatEventNetData`:
  ...
  ```
- [x] B-5b. **이벤트 생성 불변식**: 1 action → 최대 1 event. 4인 최대: 4 main + 4 sub + 4 defense + 4 death = 16
- [x] B-5c. `CombatResolutionBatchNetData : INetworkSerializable` — **수동 요소별 직렬화** (B4 해결):
  ```csharp
  public void NetworkSerialize<T>(BufferSerializer<T> s) where T : IReaderWriter
  {
...
- [x] B-5d. Overflow → state 적용 전 abort → match 강제 종료 (재시도 안 함)
- [x] B-5e. 1v1: 기존 Resolve() + CombatResultData 보존

### B-6. Balance

- [x] B-6a. 모든 랜덤 경로에 MaxRandomItems cap + GetRandomSlotCount() 헬퍼 (Codex 4→6→8/10 승인)
- [x] B-6b. 바람막이 Multi 1-use: InitializeBasicItems에 IGameModeRule → `IsUnlimited=false, RemainingUses=1` + ResetForNewRound 수정
- [x] B-6c. 타로: Multi ItemDropTable 생성 시 필터링 (GetRuleAwareDropTable 공유 헬퍼)
- [x] B-6d. Deathmatch: `min(Rule.DeathmatchGrantCount, cap - currentRandom)` via GrantRandomItems 3-param overload
- [x] B-6e. 동시 Ready 판정 = 동일 ReadyServerTick (NetworkManager.ServerTime.Tick) — B-1에서 구현 완료

### B-7. TurnManager N인

- [x] B-7a. 동적 배열 — PlayerState[] 고정 2→동적. **1v1 영향:** 배열 크기 2로 동일, 인덱싱 로직 seat 기반 전환 ✅ Codex 9/10
- [x] B-7b. PrepPhase N명 루프 — **공유 코드 변경:** for문 상한 N, Ready barrier N명 대기, 타이머 로직 공유. 1v1은 N=2로 동작 동일해야 함 ✅ Codex 9/10
- [x] B-7c. AttackPhase: Multi → snapshot + ResolveMulti + Applicator, 1v1 → **기존 CombatResolver.Resolve() 보존** (IGameModeRule 분기) ✅ Codex 8/10
- [x] B-7d. BuffDebuffSystem N인 확장 — **공유 코드 변경:** seat 순회 N명. 1v1은 2-seat 순회로 기존 동작 보존 필수 ✅ Codex 9/10
- [x] B-7e. **1v1 격리 조건:** B-7a~d의 공유 코드 변경 후 반드시 2인 Prep→Attack→Resolution→RoundReset 전체 사이클 회귀 테스트 ✅ 코드 검증 완료 — 1v1 else 분기 무변경 확인

### B-8. 통합 테스트

- [x] B-8a. 3인 드래그-타겟 → 전투 ✅ ActionQueue.TargetSeat→BuildActionIntents→ResolveMulti 전달 확인
- [x] B-8b. 미니게임 타겟 보존 ✅ QueuedAction.TargetSeat 1회 설정 후 미변경, PrepPhase 미간섭
- [x] B-8c. 중간 사망 → 행동 취소 ✅ ResolveMulti LifeState!=Alive 체크 (⚠️ 발견 이슈: delayed buff 사망 시 TryKill 미호출 → Phase C에서 수정 필요)
- [x] B-8d. Target Ghost → 취소 ✅ ResolveMulti Ghost/IsDead 타겟 스킵 확인
- [x] B-8e. 5킬 + 공동 승리 (WinnerMask Shell) ✅ EvaluateRoundEnd alive/ghost 카운트 + SurvivorMask byte 비트마스크
- [x] B-8f. 밸런스: 바람막이 1-use, 타로 필터, 초기 2개, Deathmatch cap ✅ 코드 검증 (런타임 확인 필요)
- [x] B-8g. DTO: 수동 직렬화 정상 + MTU ✅ MaxEvents=16 MaxSeats=4 검증 + 최대 ~468bytes < 1300 MTU
- [x] B-8h. 1v1 회귀 (조건부 안전 — 전체 체크리스트 코드 검증 통과) ✅
  - [x] B-8h-1. 1v1 seat resolution: MatchRoster 2-seat 생성 + TryGetSeatByClientId dict lookup 정상
  - [x] B-8h-2. 2인 PrepPhase: N루프 N=2 + Ready barrier _players.Length 순회 + 공유 타이머 정상
  - [x] B-8h-3. 2인 AttackPhase: else 분기 → CombatEngine.ResolveCombat 호출 확인 (ResolveMulti 미사용)
  - [x] B-8h-4. 2인 round reset: else→ResetPlayersForNewRound(p1,p2) + FanSpeed=DEFAULT + IsFanActive=false + BuffSystem 2-param
  - [x] B-8h-5. 1v1 disconnect: DisconnectDispatcher.SetHandler(roster) Multi 전용 → 1v1은 PSM 기본 즉시 Despawn
  - [x] B-8h-6. Multi→Lobby→1v1: MCR.OnDestroy→RestoreDefaultIfCurrent + NSC.OnNetworkStopped→_selectedMode=OneVsOne
  - [x] B-8h-7. 1v1 Rematch: BootstrapNewMatch 상태만 리셋, seat/callback 재등록 없음, OnNetworkSpawn 1회 등록
  - [x] B-8h-8. 공유 UI: GameDataBridge Dict<byte,SeatSnapshot> seat 기반, RoundResultPresenter IsMulti/else 분기
  - [x] B-8h-9. PlayerState.Initialize → SyncedPlayerIndex.Value = playerIndex, TurnManager seat=roster lookup
  - [x] B-8h-10. TrySpawnPendingClients: RosterReady false→return + GetSpawnPositionBySeat + ContainsKey 중복 방지
  - [x] B-8h-11. ResolutionPhaseRoutine: no winner→PrepPhaseRoutine, round end→HandleRoundEnd→StartNextRound→BootstrapNewMatch→PrepPhaseRoutine
  - [x] B-8h-12. OnClientDisconnectForBarrier→HandleDisconnect + PSM 즉시 DespawnPlayerForClient + PlayerState.OnNetworkDespawn unregister
- [x] B-8i. Host + remote 3 + 모든 seat 관점 ✅ OnMultiCombatResultClientRpc [Rpc(SendTo.Everyone)] 전체 브로드캐스트
- [x] B-8j. PlayerStateDelta: FanSpeed/IsBasicBlocked 정상 적용 ✅ MultiCombatApplicator.Apply delta.NewFanSpeed/IsBasicBlocked 처리 확인

---

## Phase C — N인 VFX + 환경

- [x] C-1a. PresentationBarrier (이미 N인) ✅ HashSet<ulong> + IEnumerable<ulong> — N인 지원 완료
- [x] C-1b. CombatVFXManager visual slot 매핑 ✅ Codex 8/10
- [x] C-1c. 순차 연출 ActionOrder ✅ Codex 8/10
- [x] C-1d. 수동 직렬화 DTO 소비 ✅ Codex 8/10
- [x] C-2a. 사망/Ghost 연출 (alpha 0.4) ✅ Codex 8/10
- [x] C-2b. 라운드 종료 연출 + 킬 스코어 ✅ Codex 8/10 (공동승리 판정은 C-4 통합테스트에서 추적)
- [x] C-3a. 잼민이 N스틸 ✅ 기존 N루프 + null guard 추가
- [x] C-3b. 앰뷸런스: 최저 1명 (동률 → seat index 최소) ✅ Codex 8/10 — Ghost 제외, null guard
- [x] C-3c. 폭염경보: 오름차순 ✅ BuildActionOrder HeatWave temp-first sort
- [x] C-3d. 기타: 전원 동일 ✅ 기존 N루프 무변경
- [x] C-3e. EnvironmentRuleService + IGameModeRule ✅ DetermineAmbulanceTargetMulti + Multi RPC
- [x] C-4a~d. 통합 테스트 ✅ Codex 9/10 — BLOCKER 1~8 전부 수정, 상태 전이 2단계, null/Ghost guard 전수, 승자 판정 일치
...
- [x] D-1a. FanSpeedMultiplier=1f, RecoveryMultiplier=1f 추가 ✅
- [x] D-1b. ApplyFanTick: `speed = FanSpeed.Value * modifiers.FanSpeedMultiplier` ✅
- [x] D-1c. ApplyRecoveryTick: `rate = recoveryRate * modifiers.RecoveryMultiplier` ✅
- [x] D-1d. **ChillAura epoch** ✅ ExpireChillAuras(currentTurn) at PrepPhase start, appliedTurn+1 비교:
  ```
...
- [x] D-2a. **쿨다운 저장소**: `Dictionary<(byte seat, byte skillIndex), int>` (스킬별 개별 쿨다운) ✅ MatchNetworkState.ServerGetCooldown/ServerSetCooldown/ServerTickAllCooldowns
  - skillIndex: 0=FrostStrike, 1=ChillAura
  - PrepPhase 시작 시 전원 쿨다운 1 감소
  - 라운드 리셋 시 전체 클리어
- [x] D-2b. **쿨다운 UI 복제** ✅ `NetworkList<GhostCooldownNetData>` in MatchNetworkState + Awake/Spawn/Despawn/Destroy lifecycle:
  ```csharp
...
- [x] D-2c. GhostSkillServerRpc 6단계 검증 ✅ UseGhostSkillRpc: IsServer, sender→seatLookup, Ghost, PrepPhase, target Alive, cooldown==0
- [x] D-2d. 디버프 덮어쓰기 ✅ TryUseChillAura: RevertChillAura→Remove→새 설치+entry
- [x] D-2e. **Active debuff dictionary** ✅ GhostSkillService._activeDebuffs Dictionary<byte, GhostDebuffEntry>
  - `GhostDebuffEntry { byte GhostSeat, byte SkillIndex, int AppliedTurn }`
  - **ChillAura 만료 시**: modifier 원복 **+ debuff entry 제거**
  - 라운드 리셋 시 전체 클리어
- [x] D-2f. FrostStrike ✅ 15° damage + TryKill(GhostFrost) + EvaluateRoundEnd → HandleRoundEnd
- [x] D-2g. ChillAura ✅ FanSpeedMultiplier=2, RecoveryMultiplier=0.5 + ExpireChillAuras at PrepPhase

### D-3. Ghost UI

- [x] D-3a. Ghost PrepPhase UI: 스킬 버튼 2개 ✅ GameHudBuilder.BuildGhostSkillPanel
- [x] D-3b. 타겟 선택: Eligible 생존자 클릭 ✅ GhostSkillPresenter.FindClickedAliveSeat + distance sort
- [x] D-3c. 쿨다운 오버레이 (NetworkList 읽기) ✅ GhostCooldowns.OnListChanged subscription
- [x] D-3d. PrepPhase 자유, Ready 불필요 ✅ MatchHudPresenter hides Ready for Ghost
- [x] D-3e. Ghost 시야에 전원 상태 ✅ OpponentBarPresenter already shows all remote players via NetworkVariable replication

### D-4. 통합 테스트

- [x] D-4a. FrostStrike → PrepPhase 즉시 사망 → FlushDeathQueue → 킬 귀속 ✅ Code verified
- [x] D-4b. ChillAura → fan ×2 확인 → 간접 킬 ✅ ActiveDebuffs → DamageSource.Create(GhostChill)
- [x] D-4c. Recovery ×0.5 확인 ✅ RecoveryMultiplier = 0.5f
- [x] D-4d. 덮어쓰기: 2 Ghost 동일 타겟 ✅ RevertChillAura + replace
- [x] D-4e. 라운드 종료 → 전원 Alive + 전체 초기화 ✅ ClearAll + ServerClearAllCooldowns
- [x] D-4f. Ghost 5킬 승리 ✅ ServerAddKill(source.AttackerSeat) for GhostFrost/GhostChill
- [x] D-4g. **ChillAura 만료**: 다음 PrepPhase 종료 시 해제 + debuff entry 제거 ✅ ExpireChillAuras
- [x] D-4h. **쿨다운 UI**: FrostStrike/ChillAura 개별 표시 ✅ GhostSkillPresenter + OnListChanged
...
- [x] E-1a. Solo 버튼 → 난이도 ✅ BotDifficulty enum (Easy/Normal/Hard)
- [ ] E-1b. Bot Standalone build 생성 (requires manual Unity Build Settings)
- [x] E-1c. 실행: `-batchmode -nographics` ✅ SoloMatchStarter.LaunchBotProcess
- [x] E-1d. **포트 조회** ✅ SoloMatchStarter.GetBoundPort (ephemeral port 0)
- [x] E-1e. BotBootstrap.cs ✅ command-line --port --token → Transport → Client 연결
- [x] E-1f. Connection approval: session-token ✅ SoloMatchStarter.ApproveBot validates token
- [x] E-1g. 프로세스 관리 ✅ Process.Start → 10s timeout, Kill on StopSoloMatch
- [x] E-1h. Auth 미사용, Lobby 미경유 ✅ Direct UnityTransport localhost

### E-2. Bot AI

- [x] E-2a. IBotBrain BT ✅ BotBrain MonoBehaviour with scoring
- [x] E-2b. BotPlayer: PrepPhase → BT → SelectItemServerRpc → delay → ReadyServerRpc ✅
- [x] E-2c. 난이도 3종 ✅ Easy/Normal/Hard with different scoring + delays
- [x] E-2d. Ready 타이밍 ✅ GetReadyDelay per difficulty
- [x] E-2e. 미니게임 IsBot 자동 성공 ✅ OnMiniGameStart → AutoSubmitMiniGame coroutine

### E-3. 외형/이름

- [ ] E-3a. Bot 이름 (deferred — requires CosmeticDataNV integration)
- [ ] E-3b. 기본 캐릭터 (deferred — requires CosmeticDataNV integration)
...
| Disconnect callback 경쟁 (v5.4→v5.7) | PSM 직접 callback 모든 모드에서 제거, DisconnectDispatcher 단일 destructive handler |
