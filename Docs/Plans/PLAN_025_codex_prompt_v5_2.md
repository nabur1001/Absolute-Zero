# PLAN_025 v5.2 Verification Prompt — Codex Review #7
...
11. **`ItemEffectOutcome.cs`**: Full struct with all NV-write fields (UserHeal, UserDamage, TargetHeal, TargetDamage, BlockTargetBasics, WriteUserFanSpeed, etc.)
...
## Review #6 Fixes Applied in v5.2

| # | Issue | Fix |
|---|-------|-----|
| B1 | DontDestroyWithOwner=false → NGO despawns before callback | **DontDestroyWithOwner=true** on Player.prefab. NGO transfers ownership to server → OnClientDisconnected reads live NVs → SeatRuntimeState → explicit Despawn+Destroy |
| B2 | Disconnected seat can't be targeted by effects during grace | **ISeatStateAccessor** interface: PlayerState exists→NV, absent→SeatRuntimeState. Effects stay seat-keyed in central system |
| B3 | ItemEffectRuleSnapshot formulas wrong | **Equalize**: `EqualizeToUserTemp` flag, Resolver: target.temp=user.temp. **Recovery**: `HealPerUse[]`+`MaxUses`, Resolver: `HealPerUse[MaxUses-RemainingUses]`. **Tarot**: `RequiresTargetReady`, Resolver: checks `IsReady` state |
| P1 | ReadyServerTick uint→int | NetworkTime.Tick is `int`. Fixed to `int` |
| P2 | DTO writer-side validation missing | Writer: null checks + count≤length for all arrays |
| P3 | Token: Guid predictable + no ready gate | `RandomNumberGenerator.GetBytes(32)` + `ConnectedCount==Required && TokenReadyCount==Required` |
| P4 | Prompt had 69 checkboxes, plan has 180+ | Full plan text included verbatim below |

---

## YOUR TASK

Review the **FULL PLAN** below. Verify:

1. **NGO 2.11.2 API correctness**: NetworkVariable ownership/permissions, Rpc patterns, NetworkList<T> constraints (unmanaged+IEquatable+INetworkSerializable), BufferSerializer, DontDestroyWithOwner behavior
...
4. **Disconnect/reconnect correctness**: DontDestroyWithOwner=true timing, SeatRuntimeState extraction from LIVE PlayerState, ISeatStateAccessor for grace period, hydrate on reconnect
5. **Formula accuracy**: Equalize (target.temp=user.temp, NOT average), Recovery (HealPerUse[] lookup, NOT baseHeal*remaining), Tarot (IsReady state, NOT intent)
6. **DTO serialization**: Writer+reader validation, manual element serialization, MTU bounds
7. **Token security**: RandomNumberGenerator, ready gate, Lobby permission model
...
10. **Completeness**: All 184 checkboxes are implementable, no missing steps
...
## FULL PLAN TEXT (184 checkboxes, verbatim)

```
# PLAN_025 — Multi Mode Full Implementation (v5.2 — Codex Review #6 반영)

> **Status:** 📋 Planning (v5.2 — Review #1~#6 전체 반영, 재검증 대기)
...
- [ ] A-3c. **SeatRuntimeState** 서버 전용 struct (B1 해결):
  ...
  struct SeatRuntimeState
  {
      ...
      public ScheduledEffectSnapshot[] PendingEffects;  // defensive copy
...
- [ ] A-3f. **Reconnect** (B1 해결):
  ...
  5. SeatRuntimeState → 새 PlayerState에 hydrate (서버가 모든 NV write):
     ...
     Modifiers 복원, Inventory 복원, PendingEffects 재등록
  ...
  ```
- [ ] A-3g. **ISeatStateAccessor 인터페이스 (B2 v5.2)**:
  ```csharp
  // disconnect grace 동안 ScheduledEffect/환경/Ghost가 빈 seat을 대상으로 동작해야 함
  ...
  //
  // ⚠ effects는 중앙 시스템에 seat-keyed로 보관 (BuffDebuffSystem의 ScheduledEffect dict)
  //   PlayerState에도 SeatRuntimeState에도 중복 보관하지 않음
  ```
- [ ] A-3h. **30초 timeout**: TimedOut → SeatRuntimeState 폐기 → seat permanently empty → 생존자 재평가
- [ ] A-3i. **Start gate (P3 v5.2)**: `ConnectedCount == RequiredPlayerCount && TokenReadyCount == RequiredPlayerCount` → 씬 전환
...
- [ ] A-8h. start gate: ConnectedCount + TokenReadyCount 이중 검증 (P3 v5.2)
...
13. (v5.2) Token ready gate: ConnectedCount==RequiredPlayerCount && TokenReadyCount==RequiredPlayerCount
...
| Token ready gate (v5.2) | ConnectedCount + TokenReadyCount 이중 gate |
```
...
- B1 v5.2 (DontDestroyWithOwner=true): [assessment]
- B2 v5.2 (ISeatStateAccessor): [assessment]
- B3 v5.2 (formula corrections): [assessment]
- P1 v5.2 (ReadyServerTick int): [assessment]
- P2 v5.2 (DTO writer validation): [assessment]
- P3 v5.2 (token gate + crypto): [assessment]
