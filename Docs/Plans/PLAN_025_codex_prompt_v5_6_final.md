# PLAN_025 v5.6 Final Verification Prompt — Codex Review #12

You are reviewing a **Unity 6 (6000.3.11f1) multiplayer game implementation plan** for architecture correctness. This is a **final confirmation review** — the plan was APPROVED at 9.2/10 in Review #11. Four non-blocking documentation cleanups were applied. Verify they are correct and no new issues were introduced.
...
- **Authority model:** Server-authoritative. Clients send action selection only via Rpc.
- **NetworkVariable:** Only server/host may write `.Value`.

## Key Source Code Facts (verified)

1. **`AttackItemDataSO.cs:9`**: `public float Damage;` — NOT BaseDamage
2. **`AttackItemDataSO.cs:13`**: `public bool EqualizeToUserTemp;` — target.temp = user.temp
3. **`RecoveryItemDataSO.cs:9-14`**: `HealPerUse[]` array lookup
4. **`DefenseItemDataSO.cs:10-11`**: `BlockAmount`, `Filter`
5. **`BuffItemDataSO.cs:9-11`**: `ImmediateTempDelta`, `DelayedTempDelta`, `DelayTurns` — self, signed
6. **`DebuffItemDataSO.cs:9-12`**: same + `AttackFilter` — opponent, signed
7. **`SabotageItemDataSO.cs:9`**: `SabotageType` enum: 0=Reroll, 1=Steal, 2=BlockBasic, 3=Neutralize
8. **`SpecialItemDataSO.cs:9-12`**: `SpecialEffect`(0=FanSpeedChange,1=ExtraAction,2=RevealOpponent), `EffectValue`, `TargetsSelf`, `DelayTurns`
9. **21 assets YAML-verified**: WaterGun=DirectDamage(8°,**not Equalize**), HugTshirt=Equalize, BuldakNoodles=Buff(**Imm=0**,Delayed=+20), Cat=Sabotage(Reroll), ClawMachine=Sabotage(Steal), Screwdriver=Special(FanSpeedChange,EffectValue=2,TargetsSelf=false,Delay=0)
10. **`TemperatureSystem.cs:11`**: `DEFAULT_FAN_SPEED = 1f`
11. **`PlayerSpawnManager.cs:168`**: immediate Despawn+Destroy on disconnect — replaced by DisconnectDispatcher
12. **`MatchCompositionRoot.cs:7`**: MonoBehaviour — lifecycle owner for MatchRoster(plain class)

## Review History

| Review | Score | Status |
|--------|-------|--------|
| #1-#5 | 4.0→6.0 | Foundation fixes |
| #6 | 7.3 | DontDestroyWithOwner, ISeatStateAccessor |
| #7 | 7.9 | Full NV coverage, token gate |
| #8 | 8.2 | Disconnect race, Applicator accessor |
| #9 | 8.4 | SO mapping, SnapshotFactory |
| #10 | 8.8 | Golden test values, dispatcher lifecycle |
| #11 | 9.2 | **APPROVED** — 4 non-blocking doc cleanups noted |

## Cleanups Applied After Review #11

| # | Issue | Fix |
|---|-------|-----|
| 1 | BuldakNoodles(immediate=+20) in factory Buff assets line | → immediate=0, delayed=+20 |
| 2 | Screwdriver "FanSpeedChange? or ExtraAction" | → FanSpeedChange, EffectValue=2, TargetsSelf=false, DelayTurns=0 |
| 3 | ItemEffectKind.Special description included Cat/Claw | → Special = Tarot/Screwdriver only, Cat/Claw = Sabotage noted |
| 4 | MatchRoster.OnDestroy() in dispatcher description + invariant 33 | → MatchCompositionRoot.OnDestroy() + RestoreDefaultIfCurrent + Dispose |
| + | TargetMode/ScheduledType assignment formulas added to factory | TargetMode = IsSelfTarget?Self:SingleTarget, ScheduledType per kind |

---

## YOUR TASK

Confirm the cleanups are correct and the plan remains APPROVED. Check:
1. BuldakNoodles values match YAML (Imm=0, Delayed=+20)
2. Screwdriver values confirmed (FanSpeedChange, 2, false, 0)
3. ItemEffectKind.Special no longer mentions Cat/Claw as Special
4. All MatchRoster.OnDestroy() → MatchCompositionRoot.OnDestroy() consistent
5. TargetMode/ScheduledType formulas correct
6. No new issues introduced
7. Invariant count and content still consistent

Score 0-10. If still 9+ with no new blockers, confirm APPROVED for implementation.
...
33. **(v5.5→v5.6)** DisconnectDispatcher(DDOL)가 단일 OnClientDisconnectCallback 구독. IDisconnectHandler 교체로 모드별 분기. **MatchCompositionRoot.OnDestroy()**에서 RestoreDefaultIfCurrent + roster.Dispose()로 default(PlayerSpawnManager) 복원
...
Confirm the plan remains implementation-ready after the documentation cleanups. Output:

```
## Review #12 — PLAN_025 v5.6 (Final Confirmation)

**Score: X.X / 10**

### Cleanup Verification
1. BuldakNoodles Imm=0, Delayed=+20: [correct/incorrect]
2. Screwdriver confirmed values: [correct/incorrect]
3. Cat/Claw removed from Special: [correct/incorrect]
4. MatchCompositionRoot.OnDestroy() consistent: [correct/incorrect]
5. TargetMode/ScheduledType formulas: [correct/incorrect]
6. No new issues: [yes/no — if no, list them]
7. Invariant consistency (36 items): [correct/incorrect]

### Final Status
[APPROVED / NOT APPROVED — reason if not]
```
