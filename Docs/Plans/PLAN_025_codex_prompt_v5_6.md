# PLAN_025 v5.6 Verification Prompt — Codex Review #11
...
## Key Source Code Facts (verified by grep + YAML asset read)

These are **verified facts from the actual codebase and asset files**, not assumptions:
...
3. **`RecoveryItemDataSO.cs:9-14`**: `public float[] HealPerUse = { 7f };` — array lookup: `HealPerUse[Clamp(MaxUses - RemainingUses, 0, len-1)]`
4. **`DefenseItemDataSO.cs:10-11`**: `public float BlockAmount; public DamageFilter Filter;`
...
7. **`SabotageItemDataSO.cs:9`**: `public SabotageType SabotageType;` — enum: 0=Reroll, 1=Steal, 2=BlockBasic, 3=Neutralize
8. **`SpecialItemDataSO.cs:9-12`**: `SpecialEffectType SpecialEffect; float EffectValue; bool TargetsSelf; int DelayTurns;` — 0=FanSpeedChange, 1=ExtraAction, 2=RevealOpponent
9. **`DamageFilter` enum**: 0=Temperature, 1=Food, 2=All
10. **7 concrete subclasses**: Attack, Recovery, Defense, Buff, Debuff, Sabotage, Special
11. **21 assets YAML-verified**: WaterGun=DirectDamage(Damage=8,**Equalize=false**), HugTshirt=Equalize, BuldakNoodles=**Buff**(Imm=0,Delayed=+20), Cat=**Sabotage**(Reroll), ClawMachine=**Sabotage**(Steal)
12. **`NetworkTime.cs:65`**: `public int Tick` — int, not uint
13. **`NetworkConnectionManager.cs:1362`**: despawns BEFORE callback when DontDestroyWithOwner=false
14. **`Player.prefab`**: Currently DontDestroyWithOwner=false — must change to true
15. **`PlayerSpawnManager.cs:168-175`**: OnClientDisconnectCallback → immediate Despawn+Destroy. Replaced by DisconnectDispatcher
16. **`TemperatureSystem.cs:11`**: `DEFAULT_FAN_SPEED = 1f` — round reset must use 1f, NOT 0
17. **`RoundLifecycleService.cs:31,43-44`**: already uses `TemperatureSystem.DEFAULT_FAN_SPEED`
18. **`PlayerState.cs:24,33,42`**: IsReady, HasSelectedItem, CosmeticDataNV — NOT in ISeatStateAccessor
19. **`NetworkSessionCoordinator.cs:293`**: `StartHost()` called before Relay code distribution
20. **`MatchCompositionRoot.cs:7`**: `MonoBehaviour` — NOT NetworkBehaviour

## Previous Review History

| Review | Score | Key Issues Fixed |
|--------|-------|-----------------|
| #1 | 4.0/10 | 11 blockers |
| #2 | 6.0/10 | 5 blockers |
| #3 | 6.0/10 | 8B+6P |
| #4 | 6.0/10 | 7B+6P |
| #5 | 6.0/10 | 5B+3P |
...
| #8 | 8.2/10 | 4B+2P |
| #9 | 8.4/10 | 2B+2H |
| #10 | 8.8/10 | 1B+1H: Golden test values, dispatcher lifecycle |

## Review #10 Fixes Applied in v5.6

| # | Issue | Fix |
|---|-------|-----|
| B1 | Golden test: WaterGun=DirectDamage not Equalize, HugTshirt=Equalize, BuldakNoodles Imm=0/Delayed=+20, 10+ values were "?" | **21 assets YAML-verified** — 0 unknown values. TargetMode + ScheduledType columns added. Cat/Claw correctly mapped as Sabotage. |
| H1 | MatchRoster MB/plain class unclear, cleanup owner unspecified | MatchRoster = **plain class** (IDisconnectHandler + IDisposable). MatchCompositionRoot.OnDestroy() calls RestoreDefaultIfCurrent(roster) + Dispose(). Init failure catch also restores default. |
...
1. **NGO 2.11.2 API correctness**: NV ownership, Rpc patterns, BufferSerializer, DontDestroyWithOwner
2. **Authority model**: All mutations server-side, no client NV writes
3. **Data flow**: Snapshot→Resolver→Resolution→Applicator pipeline, no NV/MB/SO in Resolver
4. **Disconnect/reconnect**: DontDestroyWithOwner=true, DisconnectDispatcher lifecycle (DDOL→MCR.OnDestroy→RestoreDefault), ISeatStateAccessor for ALL read+write, hydrate order
5. **Formulas**: Equalize, Recovery, Tarot, Buff/Debuff signed ImmediateTempDelta
6. **SO→Snapshot From()**: 7-subclass exhaustive switch, actual field names, unknown → exception
7. **Golden test table**: All 21 asset values match YAML-verified facts above. No "?" values remain
8. **MatchCombatSnapshotFactory**: Reads via ISeatStateAccessor, IsReady connected fallback, disconnected target policy
9. **DTO serialization**: Writer+reader validation, MTU bounds
10. **Token**: RandomNumberGenerator, 2-stage gate, StartHost ordering
11. **1v1 regression**: Resolve(p1,p2) preserved, DisconnectDispatcher defaults to PlayerSpawnManager
12. **Completeness**: All 195 checkboxes implementable
13. **Invariants**: 36 invariants, no contradictions
14. **Round reset**: FanSpeed=DEFAULT_FAN_SPEED(1f), IsFanActive=false

## Scoring

- **BLOCKER (B)**: Runtime failure, desync, or data loss. Must fix.
- **HIGH (H)**: Design gap causing edge-case issues. Should fix.
- **PARTIAL (P)**: Incomplete spec. Nice to fix.
- **APPROVED**: No blockers. Implementation-ready.

Score 0-10. Target: 9+ for APPROVED.

For each issue, cite: section, contradicting fact, failure scenario.

---

## FULL PLAN TEXT (195 checkboxes — mechanically copied from plan file)

# PLAN_025 — Multi Mode Full Implementation (v5.6 — Codex Review #10 반영)

> **Status:** 📋 Planning (v5.6 — Review #1~#10 전체 반영, 재검증 대기)
...
- B1 v5.6 (21-asset YAML-verified golden test): [assessment]
- H1 v5.6 (MatchRoster plain class + MCR.OnDestroy lifecycle): [assessment]
