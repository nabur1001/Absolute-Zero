# PLAN_025 v5.5 Verification Prompt — Codex Review #10
...
1. **`AttackItemDataSO.cs:9`**: `public float Damage;` — field name is **Damage**, NOT BaseDamage
2. **`AttackItemDataSO.cs:13`**: `public bool EqualizeToUserTemp;` — Equalize sets target.temp = user.temp (NOT average)
3. **`RecoveryItemDataSO.cs:9-14`**: `public float[] HealPerUse = { 7f };` — Recovery uses array lookup: `HealPerUse[Clamp(MaxUses - RemainingUses, 0, len-1)]`
4. **`DefenseItemDataSO.cs:10-11`**: `public float BlockAmount; public DamageFilter Filter;` — field names BlockAmount + Filter
5. **`BuffItemDataSO.cs:9-11`**: `float ImmediateTempDelta; float DelayedTempDelta; int DelayTurns = 1;` — self-target, signed
6. **`DebuffItemDataSO.cs:9-12`**: `float ImmediateTempDelta; float DelayedTempDelta; int DelayTurns = 1; DamageFilter AttackFilter;` — opponent-target
7. **`SabotageItemDataSO.cs:9`**: `public SabotageType SabotageType;` — enum: Reroll, Steal, BlockBasic, Neutralize (ItemEnums.cs:9)
8. **`SpecialItemDataSO.cs:9-12`**: `SpecialEffectType SpecialEffect; float EffectValue; bool TargetsSelf; int DelayTurns;` — FanSpeedChange/ExtraAction/RevealOpponent
9. **`SpecialItemDataSO.cs:17`**: `if (!ctx.Target.IsReady.Value)` — Tarot checks `IsReady.Value` state
10. **7 concrete subclasses**: AttackItemDataSO, RecoveryItemDataSO, DefenseItemDataSO, BuffItemDataSO, DebuffItemDataSO, SabotageItemDataSO, SpecialItemDataSO
11. **21 assets verified**: Fan/Windbreaker/WarmTea/Cat(Sabotage!) + 5 Attack + 2 Buff(incl BuldakNoodles!) + 1 Debuff + 1 Defense + 3 Recovery + 3 Sabotage(incl ClawMachine!) + 2 Special
12. **`NetworkTime.cs:65`**: `public int Tick => m_CachedTick;` — Tick is `int`
13. **`NetworkConnectionManager.cs:1362`**: `if (!playerObject.DontDestroyWithOwner)` — NGO despawns BEFORE callback when false
14. **`Player.prefab`**: Currently `DontDestroyWithOwner=0` (false) — must be changed to true
15. **`PlayerSpawnManager.cs:168-175`**: `OnClientDisconnectCallback` immediately Despawn+Destroy. Must be replaced by DisconnectDispatcher
16. **`TemperatureSystem.cs:11`**: `public const float DEFAULT_FAN_SPEED = 1f;` — round reset uses 1f, NOT 0
17. **`RoundLifecycleService.cs:31,43-44`**: `player.FanSpeed.Value = TemperatureSystem.DEFAULT_FAN_SPEED;` — current code already uses 1f
18. **`PlayerState.cs:24,33,42`**: `IsReady`, `HasSelectedItem`, `CosmeticDataNV` — NOT in ISeatStateAccessor (turn-progress/cosmetic)
19. **`NetworkSessionCoordinator.cs:293`**: `StartHost(relayResult.Value.ServerData)` — called before Relay code distribution
20. **Unity Lobby permissions**: Each player can only modify own PlayerData. Host can READ Private data but NOT write others'.
21. **`MatchCompositionRoot.cs:7`**: `MonoBehaviour` — NOT NetworkBehaviour, cannot own NVs
...
| #8 | 8.2/10 | 4B+2P: Disconnect callback race, Applicator accessor bypass, Buff/Debuff snapshot, FanSpeed reset |
| #9 | 8.4/10 | 2B+2H: SO mapping field names, SnapshotFactory PlayerState dependency, handler lifecycle, invariant conflict |

## Review #9 Fixes Applied in v5.5

| # | Issue | Fix |
|---|-------|-----|
| B1 | SO mapping: Attack.BaseDamage→Damage, BuldakNoodles=Buff not Attack, Cat/ClawMachine=Sabotage not Special, SabotageItemDataSO row missing, Defense/Special fields incomplete | **7 subclass exhaustive switch** with actual field names verified from source. 21-asset golden test table. Sabotage row added with SabotageType→InventoryAction/BlocksTargetBasics/NeutralizesTarget mapping |
| B2 | MatchCombatSnapshotFactory reads PlayerState directly → null for disconnected seats | **MatchCombatSnapshotFactory** explicit: gameplay state via ISeatStateAccessor.Get*(seat), IsReady = connected ? NV : false. Disconnected target policy: apply action (target is auto-ready=no action), cancel only if Ghost |
| H1 | Disconnect handler lifecycle: no subscribe/unsubscribe contract, no lobby return restoration | **DisconnectDispatcher (DDOL)**: single OnClientDisconnectCallback subscriber, IDisconnectHandler swap pattern. MatchRoster OnDestroy restores default(PlayerSpawnManager) |
| H2 | Invariant 22 ("전체 NV") contradicts invariant 30 ("4개 제외") | Invariant 22 reworded to "authoritative gameplay state" with cross-reference to 30 |
...
4. **Disconnect/reconnect correctness**: DontDestroyWithOwner=true, DisconnectDispatcher single handler, ISeatStateAccessor for ALL read+write, hydrate commit order, **handler lifecycle on lobby return**
5. **Formula accuracy**: Equalize (target.temp=user.temp), Recovery (HealPerUse[]), Tarot (IsReady), Buff/Debuff (signed ImmediateTempDelta)
6. **SO→Snapshot mapping**: All 7 subclasses covered with correct field names (Damage not BaseDamage, BlockAmount/Filter, SabotageType, SpecialEffect/EffectValue/TargetsSelf/DelayTurns). Unknown subclass → exception
7. **MatchCombatSnapshotFactory**: All reads via ISeatStateAccessor except IsReady (connected check + fallback). Disconnected target policy clear
8. **DTO serialization**: Writer+reader validation, manual element serialization, MTU bounds
9. **Token security**: RandomNumberGenerator, 2-stage gate, StartHost ordering (token table + approval → StartHost → relay code)
10. **1v1 regression**: Existing Resolve(p1,p2) + CombatResultData preserved, DisconnectDispatcher defaults to PlayerSpawnManager
11. **Completeness**: All 195 checkboxes are implementable, no missing steps
12. **ISeatStateAccessor scope**: Authoritative gameplay state only. IsReady/HasSelectedItem/CosmeticDataNV/SyncedPlayerIndex excluded with documented offline defaults
13. **Invariant consistency**: No contradictions among 34 invariants
...
- **HIGH (H)**: Design gap that may cause issues in edge cases. Should address.
...
## FULL PLAN TEXT (195 checkboxes — mechanically copied from plan file)

# PLAN_025 — Multi Mode Full Implementation (v5.5 — Codex Review #9 반영)

> **Status:** 📋 Planning (v5.5 — Review #1~#9 전체 반영, 재검증 대기)
...
| H1 | Disconnect handler subscribe/unsubscribe + lobby 복귀 시 PlayerSpawnManager 복원 미명시 | **DisconnectDispatcher (DDOL)**: IDisconnectHandler 교체 패턴, MatchRoster OnDestroy에서 default 복원 |
...
- [ ] A-3e. **Disconnect 처리 — 단일 소유자 (B1 v5.4)**:
  ...
  **해결: Disconnect 처리 단일 소유자 + lifecycle (H1 v5.5 보강)**
  - **구조: DisconnectDispatcher (DDOL MonoBehaviour)**
    - NetworkManager.OnClientDisconnectCallback에 단일 구독
    - IDisconnectHandler 인터페이스: `void OnPlayerDisconnected(ulong clientId)`
    - 모드별 handler 교체: `SetHandler(IDisconnectHandler handler)`
    - default handler = PlayerSpawnManager (기존 즉시 Despawn 동작)
    - Multi/Solo 진입 시: MatchRoster가 handler 등록 → Capture→Roster→Despawn 경로
    - Match 종료/Lobby 복귀 시: MatchRoster.OnDestroy()에서 handler를 default(PlayerSpawnManager)로 복원
    - **lifecycle 보장**:
      ① MatchRoster 생성(GameScene_Multi OnEnable) → SetHandler(this)
      ② Match 진행 중: MatchRoster가 disconnect 처리
      ③ MatchRoster 파괴(씬 전환/Lobby 복귀) → OnDestroy에서 SetHandler(defaultHandler) 복원
      ④ 1v1 GameScene: DisconnectDispatcher의 default handler = PlayerSpawnManager (변경 없음)
    - ⚠ DDOL PlayerSpawnManager와 scene-scoped MatchRoster 수명 차이를 dispatcher가 중재
...
- [ ] B-3b. `ItemEffectRuleSnapshot` readonly struct (B3 해결 — **계산 규칙 캡처, 결과 아님**):
  ...
  **4) BuffItemDataSO** → `EffectKind = Buff`
  ...
  ```
  - Assets: Soda(immediate=-5, delayed=+15), BuldakNoodles(immediate=+20)
  ...
  **7) SpecialItemDataSO** → `EffectKind = so.SpecialEffect switch { FanSpeedChange => FanControl, _ => Special }`
  ...
  ```
  - Assets: TarotCard(RevealOpponent), Screwdriver(FanSpeedChange? or ExtraAction)

  **21개 asset golden test 기댓값 (B1 v5.5)**:
  ```
  | # | Asset | SO Type | EffectKind | 주요 필드 기댓값 |
  |---|-------|---------|------------|-----------------|
  | 1 | Fan | Attack | DirectDamage | Damage=3 |
  | 2 | Windbreaker | Defense | Defense | BlockAmount=2, Filter=? |
  | 3 | WarmTea | Recovery | Recovery | HealPerUse=[7], MaxUses=1 |
  | 4 | Cat | Sabotage | Sabotage | SabotageType=Reroll |
  | 5 | HandFan | Attack | DirectDamage | Damage=? |
  | 6 | IceCream | Attack | DirectDamage | Damage=? |
  | 7 | IcedAmericano | Attack | DirectDamage | Damage=? |
  | 8 | HugTshirt | Attack | DirectDamage | Damage=? |
  | 9 | WaterGun | Attack | Equalize | EqualizeToUserTemp=true |
  | 10 | Soda | Buff | Buff | Imm=-5, Delayed=+15 |
  | 11 | BuldakNoodles | Buff | Buff | Imm=+20 |
  | 12 | Samgyetang | Debuff | Debuff | Imm=+3, Delayed=-7, Filter=Food |
  | 13 | Mask | Defense | Defense | BlockAmount=?, Filter=? |
  | 14 | HotAmericano | Recovery | Recovery | HealPerUse=? |
  | 15 | HotPack | Recovery | Recovery | HealPerUse=[12] |
  | 16 | Smartphone | Recovery | Recovery | HealPerUse=[4,6,7] |
  | 17 | BlueTape | Sabotage | Sabotage | SabotageType=BlockBasic |
  | 18 | RedCard | Sabotage | Sabotage | SabotageType=Neutralize |
  | 19 | ClawMachine | Sabotage | Sabotage | SabotageType=Steal |
  | 20 | TarotCard | Special | Special | RequiresTargetReady=true |
  | 21 | Screwdriver | Special | FanControl | EffectValue=?, TargetsSelf=? |
  ```
  - ⚠ "?" 값은 구현 시 SO asset에서 읽어 확정 후 테스트에 하드코딩
...
25. **(v5.4)** Disconnect 처리 단일 소유자: Multi/Solo에서 PlayerSpawnManager disconnect callback 비활성화, MatchRoster가 유일한 handler. Capture→Roster→Despawn 순서 보장
...
33. **(v5.5)** DisconnectDispatcher(DDOL)가 단일 OnClientDisconnectCallback 구독. IDisconnectHandler 교체로 모드별 분기. MatchRoster OnDestroy에서 default(PlayerSpawnManager) 복원
...
### HIGH (should fix)
- **H1**: [section] — [issue] — [risk]

### PARTIAL (nice to fix)
- **P1**: [section] — [issue] — [risk]

### APPROVED items (previously fixed, now correct)
- B1 v5.5 (7-subclass exhaustive From() + field names): [assessment]
- B2 v5.5 (MatchCombatSnapshotFactory ISeatStateAccessor read): [assessment]
- H1 v5.5 (DisconnectDispatcher lifecycle): [assessment]
- H2 v5.5 (Invariant 22↔30 consistency): [assessment]
